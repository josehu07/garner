...
def parse_log(input_file):
    ...
    with open(input_file, 'r') as fi:
        for line in fi.readlines():
            ...
            if len(segs) > 5:
                action += '_' + segs[5]
            trace[tid].append(RWLockOp(
                RWLock(segs[0], segs[4]), segs[2], action))
    ...
    for tid in trace:
        ...
        for op in trace[tid]:
            if op.lock not in held_locks:
                ...
                if op.action != 'release':
                    print(f"  Error: {op.lock} already held {held_locks[op.lock]} upon: {op.mode} {op.action}")
                    exit(1)
                if op.mode != held_locks[op.lock]:
                    print(f"  Error: {op.lock} release mode mismatch: expect {held_locks[op.lock]} found {op.mode}")
                    exit(1)
